...
public class PagamentoConBancomat implements StrategyOfPayment{
	//QUESTA CLASSE CREA UN METODO DI PAGAMENTO TRAMITE BANCOMAT. ESSA VIENE POI RICHIAMATA DAL METODO RICONSEGNACAR
	//IN PARCHEGGIO SERVICEIMPL NEL CASO SI PAGHI TRAMITE BANCOMAT.
	//NEL CASO DI PAGAMENTO CON BANCOMAT, OLTRE AL DOVUTO, VI SARA' LA DETRAZIONE DI 2 EURO PER SPESE DI COMMISSIONE.
	...
	if(data_riconsegna.equals(tdate) || data_riconsegna.before(tdate)) {
		...
		total = (useDays*tariffa);
		
	}else if(data_riconsegna.after(tdate)) { 
		//Conteggio con mora per ogni giorno. Ogni giorno di mora = 20.
		useDays =(int) ChronoUnit.DAYS.between(data_inizio_usoLD, data_riconsegnaLD);
		calculatemorelessdays = (int) ChronoUnit.DAYS.between(data_riconsegnaLD, tdateLD);
		useDays = useDays + calculatemorelessdays;
		total = (useDays*tariffa ) + (calculatemorelessdays*mora);
	}
	//Al totale aggiungiamo i 2 euro per via delle spese di commissione del bancomat. 
	return total + 2;
	}
}
